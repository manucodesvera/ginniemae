...
class PdfSummarizer:
    ...
    def pdf_summarizer(self, json_data, month, year, excel_folder):
        pdf_path = f"{excel_folder}/{str(month)+str(year)}_balance_summary.pdf"
        principle_types = []
        datas = json_data
        ...
        for key in grouped_data:
            ...
            out_put.append({f"{key}": count})
            count = 0

        data = [{key: int(value)} for entry in out_put for key, value in entry.items()]
        sorted_data = sorted(data, key=lambda x: list(x.values())[0], reverse=True)
        formatted_output = [{key: '{:,}'.format(value)} for entry in sorted_data for key, value in entry.items()]

        return self.tem_excel_creation(formatted_output, pdf_path)
    ...
    def tem_excel_creation(self,out_put, pdf_path):
        ...
        return self.excel_to_pdf("data.xlsx", pdf_path)
